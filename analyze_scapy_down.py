...
def process_pcap(file_name):
             ...
             if current_time_packetout != timestamp:
                if (timestamp - current_time_packetout > 1) and current_time_packetout != 0:
                    for i in range(current_time_packetout+1, timestamp):
                        list_packetout_timestamp.append([i,0])

                current_time_packetout = timestamp
                if current_time_packetout != 0:
                    list_packetout_timestamp.append([current_time_packetout, count_packet_out_ts])
                    count_packet_out_ts = 0
    ...
    count_pkt.append(count_packet_out)
    count_pkt.append(count_flow_mod)
    plot_graph(np.array(list_packetout_timestamp), np.array(list_flowmod_timestamp), name)
...
def plot_graph(list_packetin_timestamp, list_flowmod_timestamp, file_name):
    ...
    plt.title(file_name + "\n" + "packet_out="+str(count_pkt[0])+"  flow_mod="+str(count_pkt[1]))
    plt.legend()
