...
def process_pcap(file_name):
    ...
    for pkt_data in PcapReader(file_name):
        ...
        payload = binascii.hexlify(bytes(tcp_payload)) # Convert to heximal 0a000002
                                                                  # ac100164 // IP 172.16.1.100        # 0913 // port 2323
        packet_in = re.findall(b'(040a)(.{4})(00000000ffffffff)(.{8})(0000)(.{100})(.{8})(0a000002)(.{4})(.{4})', payload)
        flow_mod = re.findall(b'(040e)(.{44})(000000000000....ffffffff)(.{118})(.{8})(.{8})(0a000002)(.{8})(.{4})(.{8})(.{4})', payload)
        ...
        if packet_in:
            ...
            for i in range(len_packet_in):
                IPport = ((packet_in[i][6]) + b':' + (packet_in[i][8])).decode() # IPaddress:port
        ...
        if flow_mod:
            ...
            for i in range(len_flow_mod):
                IPport = ((flow_mod[i][4]) + b':' + (flow_mod[i][8])).decode() # IPaddress:port
    ...
    Timestamp_DF.to_csv("./save_table/Timestamp_Table_" + name.replace(' ','') + ".csv", index=True)
    PI.to_csv("./save_table/Timestamp_PI_" + name.replace(' ','') + ".csv", index=True)
    FM.to_csv("./save_table/Timestamp_FM_" + name.replace(' ','') + ".csv", index=True)
...
def plot_graph(list_packetin_timestamp, list_flowmod_timestamp, file_name):
    ...
    pd.DataFrame(data=list_packetin_timestamp).to_csv("./save_table/b_flow_in_" + name.replace(' ','') + ".csv", index=None)
    pd.DataFrame(data=list_flowmod_timestamp).to_csv("./save_table/b_flow_out_" + name.replace(' ','') + ".csv", index=None)
